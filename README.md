...
| `--max-clients` | Max concurrent connected clients; beyond this, new connections are rejected before authentication (0 = unlimited) | `1000` |
| `--max-tunnels-per-client` | Max tunnels a single client may register (0 = unlimited) | `0` |
...
| `--shutdown-timeout` | How long to wait for in-flight HTTP/admin requests to finish during graceful shutdown before forcing them closed | `15s` |
...
| `--reserved-subdomains` | Subdomains only `admin`-role tokens may claim (requires `--require-auth`); pass an empty string to disable | `admin,api,www,status,metrics,health` |
