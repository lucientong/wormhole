...
- [拆上帝对象，Server 侧 (P3-6 批次 D)](#拆上帝对象server-侧-p3-6-批次-d)
- [拆上帝对象，Client 侧 (P3-6 批次 D)](#拆上帝对象client-侧-p3-6-批次-d)
- [数据流总结](#数据流总结)
...
| 组件 | 位置 | 职责 |
|------|------|------|
| `Server` | `pkg/server/server.go` | 组合根（composition root），装配 `TunnelRegistry`/`ProxyService`/`P2PBroker` 并持有监听器生命周期（P3-6 批次 D） |
...
| 组件 | 位置 | 职责 |
|------|------|------|
| `Client` | `pkg/client/client.go` | 组合根（composition root），装配 `RelayClient`/`P2PSession`，汇总 `Stats`，持有 inspector 及本地控制/inspector HTTP 服务，并实现两者共用的 `localForwarder`/`statsRecorder` 回调接口（P3-6 批次 D） |
...
一条路由只在注册那一刻写入 Redis、之后再也不刷新的话，即使客户端仍然在线，也会在 5 分钟后静默过期——这是 v0.6 审查中发现的最致命的 HA 缺口。`ClientSession.clusterRoutes` 记录该会话在集群侧注册过的每一条 `RouteEntry`（主子域名，以及 `RegisterTunnel` 注册的额外子域名/hostname/path 前缀），`TunnelRegistry.StartHeartbeat` 每 30 秒的心跳周期都会调用 `refreshClusterRoutes`，对这些路由逐一重新执行 `RegisterRoute`——这本质上是一次可批量/管道化的 `EXPIRE`/TTL 刷新，而不是重新申请，因为这些条目本来就属于当前客户端。

### 集群心跳（`pkg/server/tunnel_registry.go`）

P3-6 批次 D 之后，集群相关的簿记（心跳、路由、端口分配）全部收拢进 `TunnelRegistry`——`Server` 组合的三个组件之一（见[组件架构](#组件架构)）——不再直接访问 `Server` 的字段。

```
TunnelRegistry.StartHeartbeat(ctx)
  ├── 每 30 秒 → NodeHeartbeat(NodeInfo{NodeID, NodeAddr})
  │              sendHeartbeat 同时记录 StateStore 是否可达 → tunnelRegistry.stateStoreHealthy（H9）
...
**陈旧持有者回收（H10）**：`router.go` 的 `RegisterSubdomain`/`RegisterHostname`/`RegisterPath` 在返回冲突之前会先检查 `isStaleOwner`（原持有者的 `Mux.IsClosed()`）；`TunnelRegistry.registerClientRoute` 在集群侧做同样的处理，主动注销失效会话在 `StateStore` 中残留的条目。客户端网络抖动后重连，能立即拿回原来的子域名/hostname/path，而不是先收到一次瞬时的冲突错误。

**健康状态暴露（H9）**：`GET /health` 新增 `cluster: {node_id, state_store_healthy}` 字段（来自 `TunnelRegistry.StateStoreHealth()`）；一旦状态存储不可达，整体 `status` 会从 `"ok"` 降级为 `"degraded"`，监控系统无需再单独探测 Redis。

### 节点间认证（S1）

`--cluster-secret` 是集群内所有节点共享的密钥。`proxyToNode`（`pkg/server/proxy_service.go`）在每次代理转发时都会附带 `X-Wormhole-Cluster-Secret` 头；`verifyClusterSecret`（在 `ProxyService.ServeHTTP` 最前面调用）会拒绝头缺失或不匹配的请求，并在放行前剥除该头，确保它永远不会被转发给本地隧道客户端。如果没有配置密钥，则不做任何校验（向后兼容 S1 之前的单节点/未加固部署）——运行真实集群的用户应当始终配置它，否则网络中的其他节点可以伪造 `X-Wormhole-*` 代理头。
...
- **服务端**（`pkg/server/proxy_service.go`）：`forwardHTTP` 里的 HTTP 响应体拷贝，以及 `handleWebSocket` 里 WebSocket 代理的两个方向。
...
## 拆上帝对象，Server 侧 (P3-6 批次 D)

P3-6 风险最高的第四个子批次，针对数据面审查发现的问题：`Server` 积累了大约 15 项不同的职责在同一个约 2200 行的结构体里（路由、客户端会话簿记、TCP 端口分配、集群心跳、HTTP/WebSocket/TCP 转发、流预算统计、P2P 信令全都挂在同一个 receiver 上）。考虑到风险，这个子批次本身又拆成两个独立验证的检查点：server 侧（本节）和 client 侧（`RelayClient`/`P2PSession`，见[下文](#拆上帝对象client-侧-p3-6-批次-d)）。

### 三个新组件

`Server` 现在是一个组合根：`NewServer` 构造三个独立组件并把它们接线在一起，但不再直接持有它们的状态。

```
NewServer(config)
  ├── registry := newTunnelRegistry(config)                                    // TunnelRegistry
  ├── metrics, auditLogger, authenticator, rateLimiter, tlsManager, adminAPI    // 不变
...
| `ProxyService` | `pkg/server/proxy_service.go` | HTTP/WebSocket/TCP 转发（即 `http.Handler`）、全局/单客户端并发流预算（DP-03/27）、池化缓冲拷贝辅助函数（DP-11） |
| `P2PBroker` | `pkg/server/p2p_broker.go` | `wormhole connect` 的 offer/result 处理、NAT 兼容性判断、端口预测候选生成 |

每个组件的依赖都通过显式构造参数传入（`Config`、需要调用的其他组件、`*Metrics`、`*Stats`、`serverCtx func() context.Context`），而不是伸手进共享的 `*Server`。`ProxyService`/`P2PBroker` 依赖 `TunnelRegistry` 时用的是它的 `TunnelRegistry` 接口而非具体结构体——这个接口刻意做得很小（消费者侧接口原则，审查报告推荐的 Go 惯用法之一），只暴露转发/信令调用方真正需要的方法（`ResolveLocal`、`ResolveRemote`、`IsLocalNode`、`FindPeerBySubdomain`、`AllocatePort` 等），不是 registry 的全部内部接口。

`admin.go` 的 `/health`、`/stats`、`/clients`、`/tunnels` 也做了同样的改动：现在调用 `registry.StateStoreHealth()`、`registry.AllocatedPorts()`、`registry.ActiveRoutes()`、`registry.Snapshot()`，而不是绕过 `Server` 直接抓它原来的 `stateStoreHealthy`/`portAllocator`/`router`/`clients`+`clientLock` 字段——这是拆分带来的实际收益的一个具体例子：管理 API 的读取路径现在完全不需要知道 `Server` 的内部加锁方式。

原来的 `handler.go`（主要是 `HTTPHandler` 的方法）和 `cluster.go`（心跳/路由辅助函数）已删除，内容分别吸收进 `proxy_service.go` 和 `tunnel_registry.go`，而不是继续作为独立文件保留——因为已经没有任何组件外部代码会调用它们了。

### 顺带的加固：`validateClusterNodeAddr`

把 `proxyToNode` 迁入 `proxy_service.go` 时，因为它和 `ServeHTTP` 落到了同一个文件里，让 linter 的 taint 分析器重新连上了一个既有的 gosec SSRF 发现（G704）——此前拆在两个文件里恰好挡住了分析器把 `route.NodeAddr` 和 `httputil.NewSingleHostReverseProxy` 连成一条链路。查阅 gosec 自身源码（`analyzers/ssrf.go`）确认 G704 **完全没有注册任何 sanitizer**——按工具自身的设计，再怎么加校验代码都不可能让这个发现自动消失。没有直接默认打 `#nosec`，而是让 `proxyToNode` 先调用 `validateClusterNodeAddr`：先用 `net.SplitHostPort` 加一层字符类校验，拒绝任何不是纯净 `host:port` 的输入，再用校验过的组件（`net.JoinHostPort`）而不是原始字符串来构造代理目标。这是真正的纵深防御——它能挡住一个被篡改或损坏的 `StateStore` 条目把 scheme、用户信息、路径或 query 组件夹带进出站请求——留下的、限定在单行的 `#nosec G704` 注释记录的是*为什么*这个抑制无法避免，而不是简单地把工具堵上。

### 测试暴露出的两个真实 bug

把测试套件的 server 构造器统一改成走 `NewServer(config)`（以获得与生产环境一致的装配）而不是手搭 `&Server{...}` 结构体字面量，过程中暴露了两个原本被更紧耦合的测试写法掩盖掉的真实 bug：

1. **跨节点路由测试里的 config 快照过期**：`TunnelRegistry` 在构造时按值拷贝了一份 `Config`。某个集群测试在构造完 server *之后*才修改 `serverA.config.ClusterNodeAddr`（因为要指向一个 `httptest.Server`，其地址在监听启动之前是未知的），这个修改不会再同步到 `registry.cfg.ClusterNodeAddr`，导致之后注册的路由携带的 `NodeAddr` 是空字符串，跨节点请求全部 404。生产代码从不会在启动之后修改 `ClusterNodeAddr`，所以这是一个纯测试修复（在地址已知后显式重新同步 `registry.cfg.ClusterNodeAddr`），不是产品 bug——但这是拆分带来的直接后果，值得记录下来供之后维护集群测试的人参考。
2. **并发流预算测试里的重复计数**：两个流上限测试各自构造了一个*独立*的 `proxyService`（通过 `newProxyService(...)`）作为被测对象，却又在原 server 的 `server.proxy` 上调用 `tryAcquireStreamSlot` 去"占用"配额——两个完全不同的计数器，占用的配额自然不会反映到被测的 handler 上，断言在解引用一个 `nil` 的 release 函数时直接 panic。修复方式是把占用配额的调用改到测试实际调用 `ServeHTTP` 的那个 `proxyService` 实例上。

### 验证

`go build ./...`、`go vet ./...`、`golangci-lint run ./...`（0 issues）、`gosec -exclude=G115 -exclude-dir=web -exclude-dir=pkg/proto/pb ./...`（0 issues，与 CI 完全同参数）、`go test -race ./...` 全部通过；`pkg/server` 覆盖率维持在 78.0%，与拆分前基本没有变化。

---

## 拆上帝对象，Client 侧 (P3-6 批次 D)

批次 D 的第二个检查点，把同样的方法论用到客户端这个"镜像版"的上帝对象上：`Client`（`pkg/client/client.go`）已经膨胀到约 2100 行，涵盖控制面连接/鉴权/重连、多隧道注册、心跳，以及整个 `wormhole connect` P2P 子系统（NAT 探测、ECDH 密钥交换、打洞、P2P 数据面）——全部由同一个 `c.mu` 保护。

### 两个新组件

`Client` 现在是一个组合根：`NewClient` 构造 `RelayClient` 和 `P2PSession`，并通过少量回调把它们接在一起，不再直接持有它们的连接/会话状态。
...
| 组件 | 文件 | 持有 |
|------|------|------|
| `RelayClient` | `pkg/client/relay_client.go` | 控制面的 `net.Conn`/`tunnel.Mux`、`connected` 标记（原子操作）、鉴权与 token 刷新、单/多隧道注册及 `activeTunnels` map、心跳 goroutine、重连循环（`Run`） |
| `P2PSession` | `pkg/client/p2p_session.go` | P2P 的 `net.PacketConn`/`*p2p.UDPMux`、ECDH `KeyPair`/`SessionCipher`、`mode` 标记（原子操作，relay 还是 P2P）、打洞尝试（`attemptP2P`）、`wormhole connect` 本地监听器（`startConnectListener`/`proxyConnectConn`） |

两个组件都只通过 `Client` 实现的两个小型消费者侧接口依赖它——`localForwarder`（把一条流交给上层去转发到本地服务）和 `statsRecorder`（把字节数/连接数上报到汇总的 `Stats`）——因此 `RelayClient`、`P2PSession` 都不需要知道 `Client` 这个具体类型的存在。`P2PSession` 回调 `RelayClient` 时也只经过最小化的 `RelayChannel` 接口（通过控制连接发送一次 P2P 结果），而不是 `RelayClient` 的完整接口。`Client` 自己保留的 `c.mu` 现在只保护它直接持有的状态（本地控制/inspector HTTP 服务）；连接状态归 `relay.mu`，P2P 会话状态归 `p2p.mu`。

### 测试暴露情况

和 server 侧检查点不同，把 `client_test.go`、`control_test.go` 迁移成统一走 `NewClient(config)` 构造（而不是手搭 `&Client{...}` 字面量）并没有暴露出任何行为性 bug——客户端原来的加锁方式本身就相对自洽。工作量主要是机械性的：改字段/方法的访问路径（例如 `c.mux` → `c.relay.mux`、`c.p2pManager` → `c.p2p.manager`），以及根据每个 `c.mu.Lock()`/`Unlock()` 代码块实际保护的是哪个组件的状态，把它重新指向 `c.relay.mu` 或 `c.p2p.mu`。

### 验证

`go build ./...`、`go vet ./...`、`golangci-lint run ./...`（0 issues）、`gosec -exclude=G115 -exclude-dir=web -exclude-dir=pkg/proto/pb ./...`（0 issues）、`go test -race ./...` 全部通过；`pkg/client` 覆盖率从 70.8% 小幅提升到 71.5%。至此 P3-6 批次 D（server + client）全部完成。
