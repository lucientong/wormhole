...
P3-6 风险最高的第四个子批次，针对数据面审查发现的问题：`Server` 积累了大约 15 项不同的职责在同一个约 2200 行的结构体里（路由、客户端会话簿记、TCP 端口分配、集群心跳、HTTP/WebSocket/TCP 转发、流预算统计、P2P 信令全都挂在同一个 receiver 上）。考虑到风险，这个子批次本身又拆成两个独立验证的检查点：server 侧（本节）和后续的 client 侧（`RelayClient`/`P2PSession`，尚未开始）。
