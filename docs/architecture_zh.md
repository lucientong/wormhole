...
ProxyService.ServeHTTP(r)
  ├── verifyClusterSecret(r) → 若配置了 --cluster-secret 且请求头缺失/不匹配则拒绝
...
`--cluster-secret` 是集群内所有节点共享的密钥。`proxyToNode`（`pkg/server/proxy_service.go`）在每次代理转发时都会附带 `X-Wormhole-Cluster-Secret` 头；`verifyClusterSecret`（在 `ProxyService.ServeHTTP` 最前面调用）会拒绝头缺失或不匹配的请求，并在放行前剥除该头，确保它永远不会被转发给本地隧道客户端。如果没有配置密钥，则不做任何校验（向后兼容单节点/未加固部署）——运行真实集群的用户应当始终配置它，否则网络中的其他节点可以伪造 `X-Wormhole-*` 代理头。
