...
| **Hole Puncher** | `pkg/p2p/hole_punch.go` | ✅ Complete — UDP probe/ack with WHPP magic prefix; primary + predicted candidate endpoints are probed from the same UDP socket |
| **Port Predictor** | `pkg/p2p/predictor.go` | ✅ Complete — Delta-based prediction for symmetric NAT |
| **P2P Manager** | `pkg/p2p/manager.go` | ✅ Complete — Coordinates STUN + candidate-aware hole punch + relay fallback |
...
- Custom hostnames are validated as dot-separated DNS labels (no ports, wildcards, trailing dots, empty labels, or control characters) before registration, and are additionally rejected outright if they fall inside the server's own base domain (equal to it, or `"<label>.<domain>"`) — `Router.Route` matches the hostname table before the subdomain table, so a hostname inside the base domain would otherwise let any client hijack another tenant's subdomain, or a reserved one, without ever going through `RegisterSubdomain`'s ownership check or `isReservedSubdomain`'s protection
- Path-prefix routes must be URL-path-only values starting with `/`; traversal segments (`..`), query/fragment delimiters, backslashes, and control characters are rejected before they reach the router/state store

### Subdomain Reservation Semantics

`RegisterRoute` (both the in-memory router and the Redis-backed cluster store) is an atomic reservation with four defined outcomes: free → reserved; same client re-registering → idempotent TTL refresh; a *live* different owner → `ErrSubdomainConflict` (connection rejected); a *stale* owner (its route entry already expired) → reclaimed. The Redis implementation runs the reservation as one Lua script (`registerRouteScript`) so the routing index key, route record, and per-client route set are updated atomically rather than split across `SETNX` + `SET` round trips. A subdomain conflict — local or cluster-wide — rejects and closes the connection outright rather than merely logging it and letting the client believe (via `AuthResponse`) that it owns a subdomain it doesn't actually control; the client's own reconnect logic retries afterward.
...
This does open a narrow split-brain window: while this node believes it owns a route (it's sitting in `clusterRoutes`, unsynced), a *different* node could legitimately claim the same key for real. The next `refreshClusterRoutes` retry surfaces this the moment it happens — `RegisterRoute` returning `ErrSubdomainConflict` for an entry the node thought it already owned means the cluster's shared state now disagrees with this node's belief. That's logged at `Error` (not the routine `Warn` used for an ordinary sync failure) and counted in `ClusterRouteConflictsTotal`; the other node's registration wins by construction, and this node immediately drops its stale local route from the in-memory `Router` and from `client.clusterRoutes` so it stops serving traffic for a key the rest of the cluster now routes elsewhere. `ClusterRouteSyncFailuresTotal` separately counts the more common, self-resolving case — any non-conflict registration/refresh failure, whether or not it ever turns into a conflict. Operators should still alert on `ClusterRouteConflictsTotal` being non-zero, because automatic local route removal preserves consistency but does not explain why two nodes believed they owned the same key; `ClusterRouteSyncFailuresTotal` moving is expected during a Redis blip and only interesting as a sustained non-zero rate. Dropping the local route also corrects the matching field on the client's own `TunnelInfo` record in `client.Tunnels` (clearing just the conflicting route dimension, or removing the whole tunnel entry if none are left) — otherwise `client.Tunnels` would keep advertising a route this node no longer serves and will never retry, a "half-dead" tunnel that looks alive but receives no traffic.
...
- `wormhole_cluster_route_conflicts_total` — should be zero. Any increment means two nodes both believed they owned the same route during a state-store outage window; the losing node drops its stale local route automatically, but this should still page someone because a real split-brain window occurred.
...
| A route works locally on one node, then disappears there after `wormhole_cluster_route_conflicts_total` increments | That node detected that Redis now maps the route key to a different live owner, so it dropped its stale local route to preserve cluster consistency | Check logs for `dropped this node's local split-brain route`, inspect `/clients`, and let the affected client reconnect/register a non-conflicting route |
